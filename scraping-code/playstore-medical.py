...
def get_all_links_from_page(baseurl):
    
    driver.get(baseurl)
    more_buttons = driver.find_elements_by_css_selector(".VfPpkd-Q0XOV>.B1yxdb")
    links = []
    gp_links = driver.find_elements_by_class_name('VfPpkd-WsjYwc')
    print(len(gp_links))
    for j in gp_links:
...
all_links = get_all_links_from_page(MEDICAL_URL)

with (open('google-playstore-medical.csv', 'w+')) as new_file:
